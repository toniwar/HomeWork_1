...
/*
Эксэпшен-функция "sliceAndSum", внутри которой происходит фильтрация списка, берётся слайс в указанном диапазоне, далее каждый из оставшихся элементов увеличивается
на указанный инкремент, и возвращается сумма элементов. В функции есть 5 необязательных параметров:
-"evenOrOddFilter" (если "true" - происходит фильтрация листа);
-"isEven" (если "true" - удаляются все нечетные числа, если "false" - удаляются четные);
...
"true", "true", "10", "20", "1"
 */
fun List<Int>.sliceAndSum(evenOrOddFilter: Boolean = false, isEven:Boolean = false, fst:Int = 0, lst:Int = lastIndex, increment:Int = 0):Int {
    var range:IntRange
        return  this.filter {
            when{
                evenOrOddFilter ->
                    if (isEven) it % 2 == 0 else it % 2 != 0
